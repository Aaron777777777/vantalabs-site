...
backup = ROOT / "guardian.before-single-paragraph-finish"
if backup.exists():
...
summaries = {
    ...
    "safe-zone-alerts": ("Home and School alerts", "Guardian can notify trusted parents when a connected child device leaves Home, arrives at School, leaves School or returns Home. These safe-zone alerts cover the moments families usually care about without requiring anyone to keep checking the map."),
    ...
    "guides/how-safe-zone-alerts-work": ("How the alert is created", "A safe-zone alert starts with the phone's location, a virtual boundary around a familiar place and confirmation that the device has genuinely entered or left the area. Guardian then sends the relevant Home or School notification, subject to signal, permissions, battery settings and operating-system restrictions."),
...
for page in GUARDIAN.rglob("index.html"):
    ...
    if 'class="single-copy"' in text:
        already_done += 1
        continue

    heading, paragraph = summaries[slug]

    wrapper_match = re.search(r'<(article|div) class="(content|body)">', text)
    if not wrapper_match:
        skipped.append(slug)
        continue

    open_end = wrapper_match.end()
...
if "/* Single-paragraph detail-page layout */" not in css:
    css += r'''
...
print(f"Converted {updated} remaining Guardian pages.")
print(f"Skipped {already_done} pages already converted.")
if skipped:
...
print("Refresh set-up-child-device and several other Guardian pages.")
